...
/// beginning with the 0x19 rpu_nal_prefix. Shared across test modules: the
/// AVC decode-path test below, and the cadence tests in `dv::levels` /
/// `sidecar::rpu_bin`, which need a parseable RPU with real DM data.
#[cfg(test)]
pub(crate) const TEST_AVC_RPU_NAL: &[u8] = &[
...
mod tests {
    ...
    fn parses_real_avc_wrapped_rpu() {
        // Exercises the full AVC RPU decode path: strip the 1-byte header,
        // clear emulation prevention, validate CRC.
        let rpu = parse_avc_rpu(TEST_AVC_RPU_NAL).expect("valid AVC-wrapped RPU");
