...
in hdrprobe 1.0.0, and is the 1.0 contract: every breaking change of the cycle is batched
into this one bump, so a consumer adapts once.
...
facts outright, and 3.0 resolves them — and says per field where each value came from, so a
derived value is never mistaken for a signalled one. Second, shape: 1.0 is the last chance to
fix naming and structure without breaking anyone twice, so the contract's inconsistencies
(sentinel values, one shape under three names, booleans that needed a second field to read)
are fixed here together.

## The renames and removals, in one table

| 2.x | 3.0 | Notes |
|---|---|---|
| `elapsed_ms` | *(removed)* | Run telemetry, not file metadata; two runs over one file now serialize identically. Progress events keep their own `elapsed_ms` |
| `video_tracks[].hdr.mastering` | `hdr.mastering_display` | One name for the shared shape |
| `video_tracks[].sl_hdr.source_mastering` | `sl_hdr.source_mastering_display` | Likewise |
| `dolby_vision.profile_compat_assumed` (bool) | `compat_source` (string) | See item 2 below |
| `dolby_vision.level_derived` (bool, true-only) | `level_source` (`"declared"` / `"derived"`) | Present exactly when `level` is; declared levels, previously unmarked, are now tagged too |
| `dolby_vision.sampled`, `hdr_vivid.sampled` (bool) | `coverage` (`"sampled"` / `"full"` / `"none"`) | See item 8 below |
| `dolby_vision.l11_content` / `l11_white_point` / `l11_reference_mode` | one `l11` object (`content`, `white_point`, `reference_mode`) | The three ride one RPU block and always appear together |
| `dolby_vision.l5_assumed_canvas` (`[w, h]` array) | `{width, height}` object | The schema's only bare tuple, gone |
| sidecar `video_tracks[].codec` = `""` | *(absent)* | "Is this a sidecar" is now a missing key, not a sentinel; video inputs always carry `codec` |
| `chroma` = `"?"` (reserved code) | *(absent)* | Reserved signalling values name nothing and omit the field. No real file is affected |
| `color.matrix` = `"IPT-PQ-c2"` | `"IPT-PQ-C2"` | Respelled per SMPTE ST 2128:2023 and Dolby's specification |
...
4. **`hdr.format` drops the `(fallback)` suffix from Dolby Vision base tags.**
   ...
   whole strings — or read the new `hdr.base` field, which carries the base tag directly.
