...
| `MDP mismatch` | DV Mastering | The Dolby Vision grade's mastering display primaries disagree with the ones the base layer signals for itself. Both are meant to describe the same grading monitor, so one of the two is imprecise, but no specification requires the pair to match and nothing in the file says which side is off. It shows up in first-party Dolby content as readily as in re-encodes, so read it as a note about how the stream was authored, not an error |
